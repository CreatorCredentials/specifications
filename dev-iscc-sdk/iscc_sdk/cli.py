...
@app.command()
def register(registry_url, file_path: Path):
    ...
    with open(file_path, 'r') as file:
        ...
        header = jwt.get_unverified_header(file_content)
        payload = jwt.decode(file_content, options={"verify_signature": False})
    print(payload)
    headers = {
    'accept': 'application/json',
    'Content-Type': 'application/json',
    'Authorization': 'Bearer 2023-iscc-horvat',
    }

    data = {
        "timestamp": header["sigT"],
        "chain_id": 0,
        "block_height": 0,
        "block_hash": "",
        "tx_idx": 0,
        "tx_hash": "",
        "declarer": header["kid"][:32],
        "iscc_code": payload["iscc"],
        "message": "",
        "meta_url": "http://"+file_content[:100],
        "registrar": header["kid"][:32]
    }

    url = registry_url+'/api/v1/register'
    try:
