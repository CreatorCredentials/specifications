...
const iscc = popup.querySelector(".iscc");
...
function addGalleryItem(data) {
    ...
    img.src = data.urlRelative;
    img.classList.add("image");
    galleryItem.appendChild(img);

    const badge = document.createElement('img');
    badge.src = "liccium-landing-logo2.png";
    ...
    badge.addEventListener("mouseenter", function() {
        ...
        for (let i = 0; i < assertions.length; i++) {
            if (assertions[i]["label"] == "ISCC Metadata") {
                isccCode = assertions[i];
                break;
            }
        }

        console.log(isccCode);
        let code = isccCode["data"]["isccCode"];
        console.log(code)
        iscc.innerHTML = `ISCC Code: ${code}`;
...
document.getElementById('files').addEventListener('change', async (event) => {
    ...
    for (const file of event.target.files) {
        ...
        // Post the file to the server when reading is complete
        reader.addEventListener('load', async (event) => {
            try {
                const url = `/upload?name=${name}`;
                const response = await fetch(url, {
                    method: 'POST',
                    headers: {
                        'Content-Type': file.type,
...
// Function to add gallery items (modify as needed)
// function addGalleryItem(data) {
//     const img = document.createElement('img');
//     console.log(data["urlRelative"])
//     img.src = data.urlRelative; // Replace with the actual property from your response
